...
function getNextFriday(date) {
  ...
  const daysUntilNextFriday = dayOfWeek === 5 ? 7 : (5 + 7 - dayOfWeek) % 7;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round(differenceInMilliseconds / millisecondsPerDay) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  const differenceDays = (date - startOfYear) / (3600 * 1000 * 24);
  const weekNumber = Math.ceil((differenceDays + startOfYear.getDay()) / 7);
  return weekNumber;
}
